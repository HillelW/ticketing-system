...
function populateTable() {
  ...
  const tds = document.getElementsByClassName("priority");
  colorRows(tds);
  sort("ticket");
  sort("ticket");
}
...
function loadMore() {
  ...
  tbody.innerHTML += additionalHtml;
      ...
      while (y == 1) {
        ...
        for (ii = 0; ii < b.length - 1; ii++) {
          ...
          if (sortValue) {
            v1 =
              column === "priority"
                ? b[ii].querySelector(sortValue).innerText.trim()
                : b[ii].querySelector(sortValue).innerText.split("-")[1].trim();
            v2 =
              column === "priority"
            ...
            v1 = "priority"
              ? b[ii].innerText.trim()
              : b[ii].innerText.split("-")[1].trim();
            v2 = "priority"
              ? b[ii + 1].innerText.trim()
              : b[ii + 1].innerText.split("-")[1].trim();
          }

          v1 =
            column === "priority"
              ? v1.toLowerCase().split("\n")[0].trim()
              : v1.toLowerCase();
          v2 =
            column === "priority"
              ? v2.toLowerCase().split("\n")[0].trim()
              : v2.toLowerCase();

          if (
            column === "priority" &&
            ((j == 0 &&
              priorityOrder.indexOf(v1) > priorityOrder.indexOf(v2)) ||
              (j == 1 && priorityOrder.indexOf(v1) < priorityOrder.indexOf(v2)))
          ) {
            bytt = 1;
            break;
          } else if (
...
function sort(column) {
  if (column === "ticket") {
    ...
    document.getElementById("ticket").innerHTML =
      mode === "^" ? "Ticket Number v" : "Ticket Number ^";
